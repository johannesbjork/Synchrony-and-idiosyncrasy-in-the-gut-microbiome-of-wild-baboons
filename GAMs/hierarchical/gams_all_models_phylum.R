...
phyla <- read.csv("../Data/phylum_order.csv")$taxon
...
## All phylum (HAVE REMOVED THE 5 MOST ABUNDANT ONES)
for(y in phyla) {
  print(y)
  
  # MODEL 1
  start.time2 <- Sys.time()
  m1 <- bam(reformulate(response=y, predictors_m1), data=metadata_hosts75, nthreads=c(nthreads,1), discrete=T)
  ...
  m2 <- bam(reformulate(response=y, predictors_m2), data=metadata_hosts75, nthreads=c(nthreads,1), discrete=T)
  ...
  m3 <- bam(reformulate(response=y, predictors_m3), data=metadata_hosts75, nthreads=c(nthreads,1), discrete=T)
