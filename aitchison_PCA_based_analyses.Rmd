...
hosts_selection <- as(sample_data(ps), "data.frame") %>%
...
# Join metadata to the PCA data.frame  
pca_dat <- 
  data.frame(pca_abund$x) %>% 
  rownames_to_column("sample_id") %>% 
  left_join(select(as(sample_data(ps),"data.frame"), sample_id, hydro_year, host, season), by="sample_id") %>% 
  drop_na() %>% 
  mutate(host=as.character(host),
...
for(hy in as.character(2001:2013)) {
  for(s in c("Dry","Wet")) {
    hy_s <- paste0(hy,"_",s)
    hosts <- hosts_selection[hosts_selection$hydro_year==hy & hosts_selection$season==s,]$host
    
    hosts_cols <- data.frame(host=hosts, cols=sample(colors, length(unique(hosts))))
    pcx <- pca_dat[pca_dat$hydro_year==hy & pca_dat$season==s & pca_dat$host %in% hosts,] %>% 
      left_join(hosts_cols, by="host")
  ...
  plot_ls[[hy]] <- 
    ...
    geom_point(aes(fill=host), size=2, pch=21, color="black") +
    facet_wrap(vars(window)) +
    scale_fill_manual(values=dat$cols) +
    theme(legend.position="none",
