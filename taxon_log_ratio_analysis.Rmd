...
# We initially filtered to the best-sampled 75 host animals, but decided to re-strict the data to 2002-2010 for the GAMs as this is the time period that is most densely sampled. This resulted in 56 host animals (Fig. S2)
...
# filter the phyloseq object to contain the same date range
ps <- prune_samples((as(sample_data(ps), "data.frame") %>% filter(collection_date >= "2002-01-01",collection_date < "2010-01-01") %>% select(sample_id) %>% pull()), ps)
...
# Average samples (clr-transformed read counts) collected from the same host
# This feature table contains the mean clr-transformed relative abundances of each after taking averages for each host (so that well-sampled hosts don’t have greater weight)
index2 <- as.integer(factor(paste0(metadata_avg$host)))
ftbl_avg_clr2 <- data.frame(ftbl_avg_clr, index2)
ftbl_avg_clr2 <- as.matrix(aggregate(.~index2, ftbl_avg_clr2, mean))

metadata_avg2 <- cbind(metadata_avg,
                       data.frame(index2=index2))
rownames(ftbl_avg_clr2) <- as.character(metadata_avg2[!duplicated(index2),]$host)
ftbl_avg_clr2 <- subset(ftbl_avg_clr2, select=-index2)

# Average samples (clr-transformed read counts) collected from the same host in the same hydro_year and season
# This is the same as ftbl_avg_clr2 but for each hydrological year and season separately
index3 <- as.integer(factor(paste0(metadata_avg$host, "_", metadata_avg$hydro_year, "_", metadata_avg$season)))
ftbl_avg_clr3 <- data.frame(ftbl_avg_clr, index3)
ftbl_avg_clr3 <- as.matrix(aggregate(.~index3, ftbl_avg_clr3, mean))
metadata_avg3 <- cbind(metadata_avg,
                       data.frame(index3=index3))
rownames(ftbl_avg_clr3) <- as.character(metadata_avg3[!duplicated(index3),]$sample_id)

ftbl_avg_clr3 <- data.frame(ftbl_avg_clr3) %>% 
  left_join(select(metadata_avg3, sample_id, index3, season, hydro_year), by="index3") %>% 
  select(-index3)
...
for(host in unique(metadata_filtered$host)) {
  ...
  # here we subtract each focal taxon’s average clr-transformed relative abundances across all samples belonging to a given host from its averaged clr-transformed relative abundance across the host population
  for(row in 1:nrow(features)) {
    log_ratios_host_phy[[host]][row,]$log_ratio <- mean(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]], na.rm=T) - mean(ftbl_avg_clr2[, features$feature[row]], na.rm=T)
    
    log_ratios_host_phy[[host]][row,]$log_ratio_std <- std(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]]) - std(ftbl_avg_clr2[, features$feature[row]])
...
for(year_season_host in names(log_ratios_year_season_host_phy)) {
  ...
  # here we subtract each focal taxon’s average clr-transformed relative abundances across all samples belonging to a given host-year-season combination from its averaged clr-transformed relative abundance across the host population (in the same hydrological year and season)
  for(row in 1:nrow(features)) {
    log_ratios_year_season_host_phy[[year_season_host]][row,]$log_ratio <- mean(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]],na.rm=T) - mean(ftbl_avg_clr3[ftbl_avg_clr3$hydro_year==unique(focal_metadata$hydro_year) & ftbl_avg_clr3$season==unique(focal_metadata$season), features$feature[row],], na.rm=T)

    log_ratios_year_season_host_phy[[year_season_host]][row,]$log_ratio_std <- std(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]])-std(ftbl_avg_clr3[ftbl_avg_clr3$hydro_year==unique(focal_metadata$hydro_year) & ftbl_avg_clr3$season==unique(focal_metadata$season), features$feature[row],])
  }
}
```
...
for(host in unique(metadata_filtered$host)) {
  ...
  for(row in 1:nrow(features)) {
    log_ratios_host_fam[[host]][row,]$log_ratio <- mean(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]], na.rm=T) - mean(ftbl_avg_clr2[, features$feature[row]], na.rm=T)
    
    log_ratios_host_fam[[host]][row,]$log_ratio_std <- std(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]]) - std(ftbl_avg_clr2[, features$feature[row]])
  }
}
...
for(year_season_host in names(log_ratios_year_season_host_fam)) {
  ...
  for(row in 1:nrow(features)) {
    log_ratios_year_season_host_fam[[year_season_host]][row,]$log_ratio <- mean(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]],na.rm=T) - mean(ftbl_avg_clr3[ftbl_avg_clr3$hydro_year==unique(focal_metadata$hydro_year) & ftbl_avg_clr3$season==unique(focal_metadata$season), features$feature[row],], na.rm=T)

    log_ratios_year_season_host_fam[[year_season_host]][row,]$log_ratio_std <- std(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]])-std(ftbl_avg_clr3[ftbl_avg_clr3$hydro_year==unique(focal_metadata$hydro_year) & ftbl_avg_clr3$season==unique(focal_metadata$season), features$feature[row],])
  }
}
```
...
combine %>%  
  ...
  labs(x=NULL, y="Absolute Log fold change") + 
...
# combine to one plot
(p_top6_phy + p_bottom6_phy + plot_layout(ncol=2)) / (p_top6_fam + p_bottom6_fam + plot_layout(ncol=2))
```
...
combine_df <- bind_rows(
    ...
    (reshape2::melt(data.frame(ftbl_avg_fam_clr)) %>% 
         ...
         arrange(desc(avg), variable) %>% 
         mutate(variable=factor(variable, rev(unique(variable)))))) %>% 
    rename("Taxon"=1, "Log fold change"=2, "Taxon level"=3, "std"=4, "variance"=5,"avg_median"=6, "avg"=7, "cv"=8) %>% 
    distinct(Taxon, .keep_all=T)  
  
m <- nlme::gls(variance~avg, data=combine_df, correlation=nlme::corARMA(p=1))
pred <- data.frame(var_pred = predict(m, combine_df), avg=combine_df$avg)

p2 <- 
  combine_df %>% 
  ggplot(aes(x=avg, y=variance, color=`Taxon level`)) +
  geom_point(size=3) +
  scale_color_manual(values=c("Phylum"="#85cce5","Family"="#69e2c5")) +
  geom_line(size=1, color="black", data=pred, aes(x=avg, y=var_pred)) +
  annotate("text", label=paste0("R^2 = ",round(R2(m)["R2_pred"][[1]],2),"; P = ", round(anova(m)[2,3],4)), x=-2.5, y=2.1, size=4.5) +
  theme(legend.position="none",
        panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        panel.background=element_rect(fill="white"),
...
ps <- prune_samples(metadata$sample_id, ps)
...
# Average samples (clr-transformed read counts) collected from the same host and grp
index2 <- as.integer(factor(paste0(metadata_avg$host)))
ftbl_avg_clr2 <- data.frame(ftbl_avg_clr, index2)
ftbl_avg_clr2 <- as.matrix(aggregate(.~index2, ftbl_avg_clr2, mean))
...
for(grp in c("1.1","1.21","1.22","2.1","2.2")) {
  ...
  # print(grp)
  focal_metadata <- metadata_avg[metadata_avg$grp %in% grp,][,c("sample_id","grp","collection_date")]
  
  for(row in 1:nrow(features)) {
    
    log_ratios_grps_phy[[grp]][row,]$log_ratio <- mean(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]], na.rm=T) - mean(ftbl_avg_clr2[, features$feature[row]], na.rm=T)
    log_ratios_grps_phy[[grp]][row,]$log_ratio_std <- std(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]]) - std(ftbl_avg_clr2[, features$feature[row]])
  }
}
...
year_season <- sort(c(paste0(unique(metadata_avg$hydro_year),"_", "aWet"),
                      paste0(unique(metadata_avg$hydro_year),"_", "bDry")))
...
for(year_season_grp in names(log_ratios_year_season_grps_phy)) {
  ...
  focal_metadata <- metadata_avg[metadata_avg$hydro_year %in% str_split(year_season_grp, "\\_", simplify=T)[,1] & metadata_avg$season %in% str_split(year_season_grp, "\\_", simplify=T)[,2] & metadata_avg$grp %in% str_split(year_season_grp, "\\_", simplify=T)[,3],][,c("sample_id","grp","collection_date","hydro_year","season")]
  
 for(row in 1:nrow(features)) {
    log_ratios_year_season_grps_phy[[year_season_grp]][row,]$log_ratio <- mean(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]],na.rm=T) - mean(ftbl_avg_clr3[ftbl_avg_clr3$hydro_year==unique(focal_metadata$hydro_year) & ftbl_avg_clr3$season==unique(focal_metadata$season), features$feature[row],], na.rm=T)

    log_ratios_year_season_grps_phy[[year_season_grp]][row,]$log_ratio_std <- std(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]])-std(ftbl_avg_clr3[ftbl_avg_clr3$hydro_year==unique(focal_metadata$hydro_year) & ftbl_avg_clr3$season==unique(focal_metadata$season), features$feature[row],])
  }
}
```
...
for(grp in c("1.1","1.21","1.22","2.1","2.2")) {
  ...
  for(row in 1:nrow(features)) {
    ...
    log_ratios_grps_fam[[grp]][row,]$log_ratio <- mean(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]], na.rm=T) - mean(ftbl_avg_clr2[, features$feature[row]], na.rm=T)
    log_ratios_grps_fam[[grp]][row,]$log_ratio_std <- std(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]]) - std(ftbl_avg_clr2[, features$feature[row]])
  }
}
...
for(year_season_grp in names(log_ratios_year_season_grps_fam)) {
 ...
 for(row in 1:nrow(features)) {
    log_ratios_year_season_grps_fam[[year_season_grp]][row,]$log_ratio <- mean(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]],na.rm=T) - mean(ftbl_avg_clr3[ftbl_avg_clr3$hydro_year==unique(focal_metadata$hydro_year) & ftbl_avg_clr3$season==unique(focal_metadata$season), features$feature[row],], na.rm=T)

    log_ratios_year_season_grps_fam[[year_season_grp]][row,]$log_ratio_std <- std(ftbl_avg_clr[focal_metadata$sample_id, features$feature[row]])-std(ftbl_avg_clr3[ftbl_avg_clr3$hydro_year==unique(focal_metadata$hydro_year) & ftbl_avg_clr3$season==unique(focal_metadata$season), features$feature[row],])
  }
}
```
...
# Taxon log-ratios for each season and hydrological year
metadata_avg$hydro_year_season <- paste0(metadata_avg$hydro_year,"_",metadata_avg$season)
metadata_avg$idx <- as.integer(factor(metadata_avg$hydro_year_season,levels=c("2002_Wet","2002_Dry","2003_Wet","2003_Dry","2004_Wet","2004_Dry","2005_Wet","2005_Dry","2006_Wet","2006_Dry","2007_Wet","2007_Dry","2008_Wet","2008_Dry","2009_Wet","2009_Dry","2010_Wet","2010_Dry")))
rect_idx_season_pad <- cbind(  data.frame(hydro_year=rep(2002:2010, each=2)),
                               data.frame(season=c("Wet","Dry"), stringsAsFactors=F),
                               data.frame(xmin=NA),
                               data.frame(xmax=NA),
...
for(n in 1:nrow(rect_idx_season_pad)) {
  rect_idx_season_pad$xmin[n] <- unique(metadata_avg[metadata_avg$hydro_year %in% rect_idx_season_pad$hydro_year[n] & metadata_avg$season %in% rect_idx_season_pad$season[n],]$idx)
  rect_idx_season_pad$xmax[n] <- unique(metadata_avg[metadata_avg$hydro_year %in% rect_idx_season_pad$hydro_year[n] & metadata_avg$season %in% rect_idx_season_pad$season[n],]$idx+1)
}
